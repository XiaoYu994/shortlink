...
public interface LinkOsStatsMapper extends BaseMapper<LinkOsStatsDO> {
    ...
    @Insert("""
            ...
            (#{item.fullShortUrl}, #{item.date}, #{item.cnt}, #{item.os}, NOW(), NOW(), 0)
            </foreach>
            ON DUPLICATE KEY UPDATE cnt = cnt + VALUES(cnt), update_time = NOW()
            </script>
            """)
    void shortLinkOsStateBatch(@Param("list") List<LinkOsStatsDO> list);

    /**
     * 根据短链接查询指定日期范围内的操作系统统计
     */
    @Select("""
            SELECT tlos.os, SUM(tlos.cnt) AS count
            FROM t_link tl
