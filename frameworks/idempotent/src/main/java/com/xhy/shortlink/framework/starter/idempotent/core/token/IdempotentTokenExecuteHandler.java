...
public final class IdempotentTokenExecuteHandler extends AbstractIdempotentExecuteHandler implements IdempotentTokenService {
    ...
    /** Token 默认过期时间，单位毫秒（6秒） */
    private static final long DEFAULT_TOKEN_TIMEOUT_MS = 6000;
    ...
    public String createToken() {
        String token = Optional.ofNullable(StrUtil.emptyToNull(idempotentProperties.getToken().getPrefix()))
                .orElse(TOKEN_PREFIX_KEY) + UUID.randomUUID();
        distributedCache.put(token, "",
                Optional.ofNullable(idempotentProperties.getToken().getTimeout()).orElse(DEFAULT_TOKEN_TIMEOUT_MS));
        return token;
    }
