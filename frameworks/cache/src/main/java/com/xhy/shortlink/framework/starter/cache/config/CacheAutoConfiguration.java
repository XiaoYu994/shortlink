...
import com.xhy.shortlink.framework.starter.cache.MultistageCacheProxy;
import com.xhy.shortlink.framework.starter.cache.RedisKeySerializer;
import com.xhy.shortlink.framework.starter.cache.StringRedisTemplateProxy;
...
@EnableConfigurationProperties({
        ...
        MultistageCacheProperties.class
})
public class CacheAutoConfiguration {
    ...
    /**
     * 创建 Redis Key 序列化器，支持自定义 Key 前缀
     */
    @Bean
    ...
    /**
     * 防止缓存穿透的布隆过滤器（需配置 enabled=true 才生效）
     */
    @Bean
    ...
    /**
     * Redis 客户端代理（默认注册，当多级缓存未启用时使用）
     */
    @Bean
    @ConditionalOnProperty(prefix = MultistageCacheProperties.PREFIX, name = "enabled", havingValue = "false", matchIfMissing = true)
    ...
    /**
     * 多级缓存代理（当 framework.cache.multistage.enabled=true 时注册）
     */
    @Bean
    @ConditionalOnProperty(prefix = MultistageCacheProperties.PREFIX, name = "enabled", havingValue = "true")
    public MultistageCacheProxy multistageCacheProxy(RedisKeySerializer redisKeySerializer,
                                                     StringRedisTemplate stringRedisTemplate,
                                                     RedissonClient redissonClient,
                                                     MultistageCacheProperties multistageCacheProperties) {
        stringRedisTemplate.setKeySerializer(redisKeySerializer);
        return new MultistageCacheProxy(stringRedisTemplate, redisDistributedProperties, redissonClient, multistageCacheProperties);
    }
}
