...
public class AbstractStrategyChoose implements ApplicationListener<ApplicationInitializingEvent> {
    ...
    // 执行策略集合
    private final Map<String, AbstractExecuteStrategy> abstractExecuteStrategyMap = new ConcurrentHashMap<>();
            ...
            return abstractExecuteStrategyMap.values().stream()
                    ...
                    .filter(each -> {
                        Pattern pattern = patternCache.computeIfAbsent(
                                each.patternMatchMark(),
                                Pattern::compile
                        );
                        return pattern.matcher(mark).matches();
                    })
                    .findFirst()
